...
import type { PomodoroPhase } from '../../ui/state';
...
export type PomodoroUpdate = {
  phase: PomodoroPhase;
  minutes: number;          // Remaining minutes
  lpfHz: number;            // LPF frequency to apply
  volumeOffsetDb: number;   // Volume offset to apply
};

export type PomodoroCallback = (update: PomodoroUpdate) => void;
...
export class PomodoroController {
  private rng: () => number;
  private onUpdate: PomodoroCallback;
  ...
  constructor(rng: () => number, onUpdate: PomodoroCallback) {
    this.rng = rng;
