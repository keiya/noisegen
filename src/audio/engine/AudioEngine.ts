...
export class AudioEngine {
  ...
  // Per-channel filters
  private hpL: BiquadFilterNode | null = null;
  private hpR: BiquadFilterNode | null = null;
  private lpL: BiquadFilterNode | null = null;
  private lpR: BiquadFilterNode | null = null;
  ...
  // Crossfeed gains
  private crossfeedLtoR: GainNode | null = null;
  private crossfeedRtoL: GainNode | null = null;

  // Master
  private masterGain: GainNode | null = null;

  // Entropy
  private entropyRng: EntropyRng | null = null;
  private entropyFeeder: EntropyFeeder | null = null;

  // State tracking for idempotent updates
  private currentMode: NoiseMode = 'pink';
  private currentVolumeDb = -12;
  private currentHpFreq = 150;
  private currentLpFreq = 5000;
  private currentCrossfeedDb = -18;
  ...
  private createBiquad(type: BiquadFilterType, freq: number): BiquadFilterNode {
  ...
  setVolumeDb(db: number): void {
    if (db === this.currentVolumeDb) return;
   ...
   */
  setHpFreq(freq: number): void {
    if (freq === this.currentHpFreq) return;
    this.currentHpFreq = freq;

    if (this.ctx && this.hpL) rampFreqTo(this.ctx, this.hpL.frequency, freq);
    if (this.ctx && this.hpR) rampFreqTo(this.ctx, this.hpR.frequency, freq);
  }

  /**
   * Set low-pass filter frequency (Hz).
   */
  setLpFreq(freq: number): void {
    if (freq === this.currentLpFreq) return;
    this.currentLpFreq = freq;

    if (this.ctx && this.lpL) rampFreqTo(this.ctx, this.lpL.frequency, freq);
    if (this.ctx && this.lpR) rampFreqTo(this.ctx, this.lpR.frequency, freq);
  }

  /**
   * Set crossfeed level in dB (-Infinity to -9dB typical).
   */
  setCrossfeedDb(db: number): void {
    if (db === this.currentCrossfeedDb) return;
    this.currentCrossfeedDb = db;

    const gain = dbToGain(db);
    if (this.ctx && this.crossfeedLtoR) rampTo(this.ctx, this.crossfeedLtoR.gain, gain);
    if (this.ctx && this.crossfeedRtoL) rampTo(this.ctx, this.crossfeedRtoL.gain, gain);
  }
