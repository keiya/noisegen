...
function initBrownState(sampleRate) {
...
class NoiseProcessor extends AudioWorkletProcessor {
    ...
    currentMode = 'pink';
    targetMode = 'pink';
    crossfadeProgress = 1; // 0 = old mode, 1 = target mode
    crossfadeSamplesRemaining = 0;
    crossfadeDuration = 0.05; // 50ms
    // Entropy management
    seedQueue = [];
            ...
            case 'set-mode':
                if (msg.mode !== this.targetMode) {
                    ...
                    if (this.crossfadeProgress >= 1) {
                        ...
                        this.crossfadeSamplesRemaining = Math.floor(this.crossfadeDuration * sampleRate);
                    }
                }
                break;
        }
    ...
    process(_inputs, outputs, _parameters) {
        ...
        if (this.samplesSinceLastSeed >= this.seedRequestThreshold &&
